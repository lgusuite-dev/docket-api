...
describe('USER API SUSPEND USER ENDPOINT', () => {
  const act = 'suspend';
  let superAdmin;
  let admin;
  let user;
  ...
  const suspendUserTest = ({ type, label, users }) => {
    it(`Should NOT suspend ${label} and its child users. INVALID USER TYPE`, async () => {
      if (type === 'users') {
    ...
    if (type === 'admins') {
      it(`Should suspend ${label} and its child users`, async () => {
        const id = admin._id;
        const token = s_token;
      ...
      it(`Should NOT suspend ${label} and its child users. ADMIN ALREADY SUSPENDED`, async () => {
        const id = admin._id;
        const token = s_token;
        const session = s_session;
      ...
      it(`Should NOT suspend ${label}. USER ALREADY SUSPENDED`, async () => {
        const id = user._id;
        const token = a_token;
    ...
    it(`Should NOT suspend ${label}. NO AUTHORIZATION HEADER`, async () => {
    ...
    it(`Should NOT suspend ${label}. NO SESSION AUTH HEADER`, async () => {
    ...
    it(`Should NOT suspend ${label}. INVALID AUTH SESSION VALUE`, async () => {
      if (type === 'users') {
    ...
    it(`Should NOT suspend ${label}. DELETED USER`, async () => {
      const token = s_token;
      const session = s_session;
      const id = type === 'admins' ? admin._id : superAdmin._id;
